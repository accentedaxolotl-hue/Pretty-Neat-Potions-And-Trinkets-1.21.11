...
public class HauntedPotionItem extends PotionBase {
    public HauntedPotionItem(Item.Properties properties) { super(properties); }

    @Override
    protected void applyEffect(Player player, Level level) {
        player.addEffect(
                new MobEffectInstance(ModEffects.hauntedEffect.getHolder().get(), 1800)
        );
    }
}
