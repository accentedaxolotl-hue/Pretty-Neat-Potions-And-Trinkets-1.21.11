...
public class FishyWaterItem extends PotionBase {
    public FishyWaterItem(Properties properties) { super(properties); }

    @Override
    protected void applyEffect(Player player, Level level) {
        player.addEffect(
                new MobEffectInstance(ModEffects.fishyEffect.getHolder().get(), 60)
        );
    }
}